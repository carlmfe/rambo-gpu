#pragma once
#ifndef RAMBO_CUDA_INTEGRANDS_CUH
#define RAMBO_CUDA_INTEGRANDS_CUH
...
struct EggholderIntegrand {
    ...
    __host__ __device__ EggholderIntegrand(double lambda = 1000000.0) 
    ...
    __device__ auto evaluate(const double momenta[][4]) const -> double {
        ...
        const double arg1 = fabs((s12 - s23) / lambdaSquared);
        const double arg2 = fabs(s13 / lambdaSquared);
        return sin(sqrt(arg1)) * cos(sqrt(arg2));
    }
};
...
struct ConstantIntegrand {
    ...
    __host__ __device__ ConstantIntegrand(double v = 1.0) : value(v) {}
...
struct MandelstamSIntegrand {
    ...
    __host__ __device__ MandelstamSIntegrand(double s = 1.0) : scale(s) {}
...
struct DrellYanIntegrand {
    ...
    __host__ __device__ DrellYanIntegrand(double eq = 2.0/3.0, double alpha = 1.0/137.035999)
...
#endif // RAMBO_CUDA_INTEGRANDS_CUH
